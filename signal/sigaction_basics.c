...
static void signal_handler(int signum, siginfo_t *info, void *ucontext)
{
    ...
    if (info->si_pid == getpid()) {
        PRINT_INFO("self-raised, ignoring");
    } else {
        PRINT_INFO("external signal, exiting");
        g_is_running = 0;
    }
...
int main(void)
{
    ...
    if (sigaction(SIGKILL, &sa, NULL) == -1) {
        PRINT_PWARN(errno, "failed to register SIGKILL handler (expected; cannot be caught)");
    }

    if (sigaction(SIGTERM, &sa, NULL) == -1) {
        PRINT_PERROR(errno, "failed to register SIGTERM handler");
        return EXIT_FAILURE;
    }

    if (sigaction(SIGSTOP, &sa, NULL) == -1) {
        PRINT_PWARN(errno, "failed to register SIGSTOP handler (expected; cannot be caught)");
    }

    PRINT_INFO("loop is running (press Ctrl+C or run 'kill %d' command)", getpid());
    ...
    while (g_is_running) {
        ...
        if (loop_count >= 5) {
            loop_count = 0;
            PRINT_INFO("raising SIGINT (self-raised, expected to be ignored)...");
            if (raise(SIGINT) != 0) {
